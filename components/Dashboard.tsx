...
const DashboardCard: React.FC<DashboardCardProps> = ({ item, onNavigate }) => {
    const isActionable = ['password-manager', 'smart-accountant', 'phone-book'].includes(item.id);
        ...
        <div className="container mx-auto px-4 py-8 sm:py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {DASHBOARD_ITEMS.map((item) => (
                    <DashboardCard key={item.id} item={item} onNavigate={onNavigate} />
                ))}
            </div>
        </div>
    );
